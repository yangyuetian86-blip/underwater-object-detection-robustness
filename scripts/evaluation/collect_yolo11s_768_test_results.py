...
def main():
    ...
    for i in range(1, 6):
        print("=" * 60)
        print(f"正在评估 YOLO11s_768 test_fold_{i} ...")

        model_path = ROOT / "runs/detect" / f"yolo11s_768_train_fold_{i}" / "weights" / "best.pt"
        ...
        metrics = model.val(
            ...
            name=f"yolo11s_768_test_fold_{i}_summary",
            exist_ok=True
        )
    ...
    save_path = ROOT / "runs/detect/yolo11s_768_kfold_test_summary.csv"
    ...
    print(f"\nYOLO11s_768 test 结果已保存到: {save_path}")
